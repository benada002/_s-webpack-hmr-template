...
1. Search for `'replace-text-domain'` (inside single quotations) to capture the text domain and replace with: `'megatherium-is-awesome'`.
2. Search for `replace_function_slug_` to capture all the functions names and replace with: `megatherium_is_awesome_`.
3. Search for `Text Domain: replace-text-domain` in `style.css` and replace with: `Text Domain: megatherium-is-awesome`.
